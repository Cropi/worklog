                ...
                try:
                    ...
                    for _, obj in inspect.getmembers(module):
                        if (
                            ...
                            # Instantiate the plugin with no config
                            plugin = obj()

                            # Register the plugin by its type
                            self.plugins[plugin.get_type()] = plugin
                            log.info(f"Registered plugin for type: {plugin.get_type()}")

                except Exception as e:
                    log.error(f"Error loading plugin {module_name}: {e}")
